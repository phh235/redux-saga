const ProductCard = () => {
  ...
  const getColorStyles = (name: string) => {
    switch (name) {
      case "BLACK HORSE GOLD":
        return { nameColor: "#ceb56e", shadowColor: "#000" };
      case "ROYAL HONEY VIP":
        return { nameColor: "black", shadowColor: "#fff" };
      case "ROYAL HONEY FOR HER":
        return { nameColor: "#942d5d", shadowColor: "#fff" };
      default:
        return { nameColor: "#000", shadowColor: "#000" };
    }
  };

  return (
    <div className="w-full md:px-[30px] px-[15px] lg:px-[80px] pt-[100px]">
      ...
      <div className="md:text-center text-justify leading-[36px] font-bold text-base mb-[39px] font-[NotoSerifCJKjp-Bold]">
      ...
      <div className="mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[15px]">
          {products.map((product) => {
            const { nameColor, shadowColor } = getColorStyles(product.name);

            return (
              <div key={product.id} className="flex flex-col rounded-[10px] w-auto overflow-hidden">
                {/* Title header */}
                <div className="bg-[#7e9bae] text-white text-[17px] text-center py-[10px] font-semibold line-clamp-1">
                  {product.title}
                </div>
                ...
                <div className="bg-[#7e9bae] pt-[10px]">
                  <div className="p-[12px] bg-gradient-to-r from-[#a0b5c3] to-[#7e9bae]">
                    <div className="tracking-[-0.4px] leading-[60px]" style={{ color: nameColor }}>
                      <div
                        style={{ textShadow: `1.5px 1.5px 0px ${shadowColor}` }}
